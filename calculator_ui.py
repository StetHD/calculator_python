...
from tkinter import Menu, messagebox, CENTER
...
class CalculatorUi:
    def __init__(self, config=_config):
        ...
        self.__input = customtkinter.CTkEntry(self.__main_frame, height=120, width=358)
    ...
    def __input_validation(self, what):
        valid_inputs = ("^", "/", "(", ")", "%", "!", ".", "-", "+", "*")
    ...
    def __create_input(self):
        ...
        self.__input.configure(font=font, corner_radius=0, fg_color="#000000", validate="key")
    ...
    def __create_buttons(self):
        font = (self.__config.font["family"], self.__config.font["size"], self.__config.font["weight"])
        buttons = []
        for (row_index, buttons_row) in enumerate(self.__config.buttons):
            ...
            for (index, btn) in enumerate(buttons_row):
                button = customtkinter.CTkButton(master=self.__main_frame, width=76, height=76, corner_radius=38,
                                                 text_font=font, fg_color=btn["bg"], text="")
